...
void show_success_dialog(GtkWidget *parent_window, const char *message) {
    // Create the dialog with custom title and button
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        "Success",                 // Title
        GTK_WINDOW(parent_window), // Parent window
        GTK_DIALOG_MODAL,          // Modal dialog
    ...
    // Create a scrolled window to handle large text
    GtkWidget *scroll = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroll), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);

    // Create a text view to display the success message
    GtkWidget *text_view = gtk_text_view_new();
    gtk_text_view_set_editable(GTK_TEXT_VIEW(text_view), FALSE);
    gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(text_view), GTK_WRAP_WORD);
...
void show_error_dialog(GtkWidget *parent_window, const char *message) {
    ...
    // Create a text view to display the error message
    GtkWidget *text_view = gtk_text_view_new();
    gtk_text_view_set_editable(GTK_TEXT_VIEW(text_view), FALSE);
    gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(text_view), GTK_WRAP_WORD);
